...
def preprocess(args):
    ...
    TFv.pp.filter_and_normalize(adata, min_shared_counts=10, n_top_genes=1000, log=True) #include the following steps
    adata.X = adata.layers["total"].copy()

    adata.obs['clusters'] = adata_proc.obs['cell_type_abbr']
    adata.uns['clusters_colors'] = adata_proc.uns['cell_type_abbr_colors']
