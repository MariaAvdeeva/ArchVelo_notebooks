...
if run_aa:
    ...
    if benchmark:
        wcs = np.linspace(0,1,11)[[3]+list(set(range(11))-{3})]
    else:
        wcs = [0.3]
    ...
    if indiv:
        ...
        avel.var['fit_likelihood_u'] = np.nan
        avel.var['fit_likelihood_s'] = np.nan
        for g in avel.var_names:    
            liks = calc_lik_ArchVelo(g,
                          adata_atac = adata_atac,
                          avel = avel)
            avel.var['fit_likelihood'].loc[g] = liks[0]
            avel.var['fit_likelihood_u'].loc[g] = liks[1]
            avel.var['fit_likelihood_s'].loc[g] = liks[2]
